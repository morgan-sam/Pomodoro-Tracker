...
const ScrollingTimeline = (props) => {
  ...
  const pomodoroColor = window
    .getComputedStyle(document.documentElement)
    .getPropertyValue("--color1-mid");
  const encoreColor = window
    .getComputedStyle(document.documentElement)
    .getPropertyValue("--color1-dark");
  ...
  useEffect(() => {
    ...
    drawTimeline(
      { ...timelineOptions, context: canvasRef.current.getContext("2d") },
      generateRandomEvents()
    );
    drawTimeline(
      { ...timelineOptions, context: canvasRef2.current.getContext("2d") },
      generateRandomEvents()
    );
  }, []);

  const curTimeToEventTime = (curTime) => ({
    hour: Math.floor(curTime / 60),
    minute: curTime % 60,
  });
  ...
  return (
    <div
      className="scrolling-timeline-container"
      style={{ height: `${timelineHeight}px` }}
    >
      <canvas
        className="scrolling-timeline"
        ref={canvasRef}
        width={24 * hourWidth}
